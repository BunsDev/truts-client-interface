                        ...
                        <div className={styles.content}>
                            ...
                            </div>
                            {
                                (rid.length > 0 && selected.review_desc) && <Comment
                                    ...
                                    highlight={true}
                                    slug={slug}
                                    twitter_slug={dao_data.twitter_link.toString().split('/')[dao_data.twitter_link.split('/').length - 1]}
                                />
                            }
                            {
...
function Comment({ comment, address, rating, profile_img, openModel, data, openConnectWallet, highlight, slug, twitter_slug }) {
    ...
    const { connectAsync, connectors, isLoading, pendingConnector } = useConnect();
    const { disconnectAsync } = useDisconnect()
    const { address : isAddress, isConnected, connector, isConnecting } = useAccount();
